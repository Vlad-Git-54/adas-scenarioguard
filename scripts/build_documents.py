...
def long_paragraphs(topic: str, facts: Sequence[str], project: str) -> List[str]:
    a, b, c, d = facts
    variant = sum(ord(ch) for ch in topic) % 10
    openings = [
        f"Раздел «{topic}» уточняет, какую часть задачи ADAS нужно контролировать при работе со сложной дорожной сценой. Для этой темы важна не только правильная метка класса, но и устойчивость решения при близком объекте, частичной видимости и неоднозначной геометрии.",
        f"Тема «{topic}» связывает предметный обзор с будущим экспериментом. В работе именно «{topic}» помогает перейти от общей точности распознавания к анализу ситуаций, где цена пропуска выше обычной ошибки классификации.",
        f"Для раздела «{topic}» ключевой вопрос состоит в том, какие признаки действительно можно проверить на данных. Поэтому рассуждение о «{topic}» не сводится к описанию ADAS в целом: каждый тезис должен быть связан с измеряемой сценой и последующей оценкой модели.",
        f"Разбор темы «{topic}» строится вокруг надежности восприятия. В этой теме средняя метрика остается только ориентиром, потому что она плохо показывает редкие случаи с окклюзией, усечением объекта, неточной 3D-геометрией или сниженным качеством наблюдения.",
        f"В разделе «{topic}» акцент сделан на инженерной проверяемости. Для «{topic}» это особенно важно: если признак нельзя получить из разметки или воспроизвести в коде, он остается только общим требованием и не помогает доказать результат ВКР.",
        f"Тема «{topic}» рассматривается через scenario-level подход. Для «{topic}» единицей анализа становится не отдельный пиксель или кадр сам по себе, а дорожная сцена с набором признаков риска, надежности и неопределенности.",
        f"Для темы «{topic}» важно отделить исследовательский прототип от промышленной ADAS-системы. В рамках «{topic}» ВКР показывает воспроизводимый способ оценки критичности, но не заявляет сертифицированную безопасность автомобиля.",
        f"Раздел «{topic}» вводит практическую рамку для дальнейшего эксперимента. При проверке темы «{topic}» модель должна обучаться на реальных аннотациях, а пороги и метрики должны проверяться на отложенной выборке без ручной подгонки.",
        f"В теме «{topic}» соединяются три уровня работы: данные, признаки и интерпретация результата. Для «{topic}» такой порядок помогает не потерять связь между теоретическим описанием ADAS и тем, что реально проверяется в проекте.",
        f"Раздел «{topic}» нужен для того, чтобы заранее определить границы метода. В этой теме такой подход снижает риск завышенных выводов: сильные стороны показываются через метрики, а слабые места выносятся в анализ ошибок и ограничения.",
    ]
    factor_blocks = [
        [
            f"Риск, связанный с {a}, выделен отдельно, потому что он может исчезнуть в усредненной оценке качества. Для ADAS важны случаи, где ошибка восприятия задерживает реакцию системы, поэтому такие признаки далее участвуют в анализе false negative.",
            f"Связь с {b} показывает, почему одного confidence недостаточно. Камера, LiDAR и radar дают разные виды информации, а при расхождении признаков итоговая оценка должна объяснять источник ненадежности, а не маскировать его одной цифрой.",
            f"Ограничение, связанное с {c}, задает проверяемую границу эксперимента. Аннотации KITTI не заменяют полный raw sensor pipeline, но дают реальные классы, 2D/3D-геометрию, occlusion и truncation для честного обучения модели риска.",
        ],
        [
            f"Аспект, связанный с {a}, помогает объяснить, почему редкая сцена не должна растворяться в общей accuracy. Даже небольшое число пропущенных критических случаев может быть важнее красивого среднего значения по всему test split.",
            f"Аспект, связанный с {b}, нужен для описания надежности наблюдения. Разные сенсоры деградируют неодинаково, поэтому модель должна учитывать uncertainty и reliability явно, а не прятать сомнение внутри итогового класса.",
            f"Аспект, связанный с {c}, делает эксперимент повторяемым. Признаки рассчитываются из зафиксированной разметки KITTI, поэтому другой проверяющий может пройти тот же путь от исходных label-файлов до метрик.",
        ],
        [
            f"Положение, связанное с {a}, помогает отделить обычные сцены от случаев с более высокой ценой пропуска. Близкий или плохо видимый объект важнее для безопасности, чем далекий и хорошо наблюдаемый объект, поэтому recall и FNR рассматриваются рядом с общей точностью.",
            f"Положение, связанное с {b}, добавляет к оценке контекст наблюдения. Сцена не является статичной картинкой: часть информации может теряться из-за условий съемки, окклюзии или усечения, и это должно отражаться в признаках надежности.",
            f"Положение, связанное с {c}, переводит общую идею в реализацию. Признаки должны извлекаться автоматически, порог выбирается на validation split, а итоговая оценка проверяется на test split без ручной настройки результата.",
        ],
        [
            f"Фактор, связанный с {a}, важен для последующего анализа ошибок. Если критический случай встречается редко, средняя метрика может выглядеть приемлемо даже при заметном числе пропусков, поэтому в результатах отдельно показываются confusion matrix и error cases.",
            f"Фактор, связанный с {b}, отражает согласованность признаков наблюдения и геометрии. Когда наблюдение деградирует, модель должна переходить к более осторожной оценке, а не сохранять прежний уровень доверия к сцене.",
            f"Фактор, связанный с {c}, задает предел применимости прототипа. Работа не подменяет промышленную сертификацию, но показывает воспроизводимый способ построить и проверить слой оценки критичности на реальных дорожных аннотациях.",
        ],
        [
            f"С точки зрения данных особенно важна связь с {a}. Этот пункт определяет, какие строки scenario table будут считаться информативными для обучения и где может возникнуть риск пропуска критической сцены.",
            f"С точки зрения модели существенна связь с {b}. Если такой признак ухудшается, итоговая оценка должна становиться осторожнее, иначе высокая средняя точность будет скрывать слабые места метода.",
            f"С точки зрения проверки значимым остается {c}. Он позволяет связать текст раздела с конкретными файлами, метриками и шагами запуска, а не оставлять выводы на уровне общих рассуждений.",
        ],
        [
            f"В инженерном смысле фактор, связанный с {a}, задает один из источников неопределенности. Для выбранного проекта важно не просто перечислить этот источник, а показать, как он превращается в численный признак.",
            f"Дополнительный слой связан с {b}. Он помогает объяснить, почему одинаковая классификация объекта может иметь разный уровень доверия в разных дорожных условиях.",
            f"Практическая проверка опирается на условие, связанное с {c}. Благодаря этому методика остается ограниченной по масштабу, но прозрачной: входные данные, признаки и метрики можно восстановить из репозитория.",
        ],
        [
            f"При анализе темы «{topic}» содержательным ориентиром становится фактор, связанный с {a}. Этот ориентир нужен, чтобы оценивать не только факт обнаружения объекта, но и последствия возможного пропуска.",
            f"Следующий ориентир связан с {b}. Он связывает качество наблюдения с поведением модели и помогает отличать уверенное решение от ситуации, где требуется осторожный режим.",
            f"Еще один ориентир связан с {c}. Он не расширяет эксперимент искусственно, а фиксирует условия, при которых результат можно повторить и проверить на тех же данных.",
        ],
        [
            f"Для темы «{topic}» характеристика, связанная с {a}, выступает как проверяемый признак риска. Если ее игнорировать, часть сложных сцен будет выглядеть обычной, хотя для ADAS она требует отдельного внимания.",
            f"Характеристика, связанная с {b}, уточняет качество сенсорной информации. В работе она нужна не для усложнения текста, а для объяснения, почему модель иногда должна снижать доверие к результату.",
            f"Характеристика, связанная с {c}, связывает методику с воспроизводимостью. Она задает условие, при котором эксперимент можно повторить без скрытых ручных решений и без подгонки test split.",
        ],
        [
            f"В разделе «{topic}» тезис, связанный с {a}, помогает перейти от общей безопасности к измеряемому признаку. Такой переход важен: комиссия должна видеть, что утверждение подтверждается данными, а не только описанием предметной области.",
            f"Тезис, связанный с {b}, добавляет к модели контекст надежности. Он показывает, какие ограничения возникают у восприятия, когда разные признаки сцены дают неполную или неодинаково устойчивую информацию.",
            f"Тезис, связанный с {c}, закрепляет экспериментальную дисциплину. Подготовка данных, обучение, выбор порога и оценка должны идти по фиксированному сценарию, иначе сравнение моделей теряет смысл.",
        ],
        [
            f"Для дальнейшей реализации важно, что фактор, связанный с {a}, можно перевести в конкретное поле или расчетный признак. Это снижает разрыв между теоретическим разделом и программной частью.",
            f"Также важно, что аспект, связанный с {b}, влияет на интерпретацию результата. Одинаковый класс объекта не означает одинаковый риск, если условия наблюдения и геометрия сцены различаются.",
            f"Наконец, условие, связанное с {c}, определяет, как будет устроена проверка. Итоговый вывод должен вытекать из сохраненных артефактов, а не из устного объяснения или ручной корректировки таблиц.",
        ],
    ]
    project_blocks = [
        f"В практической части по теме «{topic}» выбранное решение {project}. Такой выбор для «{topic}» ограничивает масштаб эксперимента, но делает результат проверяемым: можно повторить подготовку данных, обучение, подбор порога и расчет метрик.",
        f"В реализации темы «{topic}» выбранное решение {project}. Благодаря этому положения раздела можно проследить в данных, коде, сохраненных моделях, графиках и таблицах результатов.",
        f"На уровне прототипа для раздела «{topic}» выбранное решение {project}. Оно сохраняет связь с реальной задачей ADAS и одновременно не подменяет промышленную валидацию исследовательским экспериментом.",
        f"В экспериментальной части темы «{topic}» выбранное решение {project}. Проверка строится так, чтобы путь от исходных аннотаций до метрик не зависел от скрытых ручных шагов.",
        f"В программной реализации раздела «{topic}» выбранное решение {project}. Это привязывает выводы к артефактам: данным, сценарной таблице, обученным весам, графикам и итоговым документам.",
        f"Для проверки положения «{topic}» выбранное решение {project}. Такой формат не обещает больше, чем реально сделано, зато позволяет повторить эксперимент и увидеть те же ограничения метода.",
        f"В проекте для темы «{topic}» выбранное решение {project}. Оно связывает теоретическую часть с командами запуска и показывает, что результаты получены на реальных аннотациях, а не на условном примере.",
        f"В итоговой методике тема «{topic}» закреплена так: выбранное решение {project}. Поэтому выводы раздела можно проверить по сохраненным файлам, а не только по текстовому описанию.",
        f"На уровне ВКР раздел «{topic}» оформлен через решение, которое {project}. Оно оставляет место для будущего raw sensor pipeline, но текущие метрики не смешивает с планируемым расширением.",
        f"В экспериментальном контуре тема «{topic}» реализована через решение, которое {project}. Такой контур удобен для защиты: по нему можно пройти от исходных данных до метрик и ошибок.",
    ]
    bridge_blocks = [
        f"Для логики раздела «{topic}» этот фрагмент важен как переход от обзора к проверяемому эксперименту. Признак, связанный с {a}, должен быть вычислен из реальной разметки, попасть в таблицу сценариев и повлиять на решение модели.",
        f"В практической части материал о «{topic}» используется как критерий инженерной состоятельности решения. Эксперимент по признаку {b} должен показать не только итоговую метрику, но и причину, по которой отдельные сцены получают высокий или низкий риск.",
        f"Смысловой акцент раздела «{topic}» сделан на прослеживаемости. Для положения о {c} должно быть понятно, какой артефакт его подтверждает: сценарная таблица, расчет признаков, validation/test split, метрика качества или список ошибок.",
        f"Для оценки темы «{topic}» важно, что раздел не подменяет обучение модели ручной классификацией. Он задает признаки и ограничения, после чего модель обучается на реальных аннотациях KITTI и проверяется на отложенной выборке.",
        f"Фрагмент о «{topic}» нужен для связи между текстом и экспериментом. Если вводится признак {a}, дальше должно быть видно, где он рассчитывается, как попадает в обучение и какую роль играет при оценке test split.",
        f"Содержательная проверка темы «{topic}» строится вокруг воспроизводимости. Вывод о {b} должен иметь опору: таблицу сценариев, файл метрик, график, список ошибок или команду запуска, которую можно повторить.",
        f"Для раздела «{topic}» такой подход уменьшает риск декоративного обзора литературы. Теоретическая часть объясняет, почему для {c} выбраны именно эти признаки, пороги и метрики.",
        f"Раздел «{topic}» также задает осторожность в формулировках. Методика работает на доступных реальных аннотациях, а утверждение о {a} не выдается за промышленную сертификацию системы.",
        f"Для защиты темы «{topic}» важно, что аргументация не опирается на скрытые действия. Подготовка данных, обучение, оценка и анализ {b} описаны так, чтобы проверяющий мог восстановить цепочку решений.",
        f"В результате раздел «{topic}» выполняет не только обзорную, но и контрольную функцию. Он показывает, как предпосылка о {c} будет проверена программно и какие ограничения остаются в текущей версии.",
    ]
    review_blocks = [
        f"При защите раздела «{topic}» удобно показать цепочку проверки: фактор, связанный с {a}, фиксируется в данных, аспект, связанный с {b}, объясняет надежность наблюдения, а условие, связанное с {c}, задает границы эксперимента. Тогда тезис «{d}» выглядит как проверяемый вывод из подготовленной scenario table.",
        f"Комиссионная проверка темы «{topic}» должна увидеть три опоры: исходные аннотации, расчет признаков и сохраненные метрики. В этой логике фактор, связанный с {a}, описывает предметный риск, аспект, связанный с {b}, показывает источник неопределенности, а условие, связанное с {c}, помогает отделить доказанный результат от будущего расширения.",
        f"Для раздела «{topic}» важно заранее снять возможный вопрос о ручной подгонке. Если признаки, связанные с {a} и {b}, формируются до обучения, а условие {c} проверяется на отложенной выборке, то утверждение «{d}» сохраняет инженерную прозрачность.",
        f"Сильная сторона раздела «{topic}» состоит в том, что он связывает предметную область с конкретными файлами проекта. Фактор, связанный с {a}, должен быть виден в признаках; аспект, связанный с {b}, — в интерпретации риска; условие, связанное с {c}, — в воспроизводимости эксперимента.",
        f"Для темы «{topic}» отдельного внимания заслуживает язык выводов. Здесь нельзя обещать промышленную безопасность: корректнее показать, как факторы, связанные с {a}, {b} и {c}, подтверждают ограниченный, но честно проверенный прототип.",
        f"Проверяющему по разделу «{topic}» важно увидеть, что модель действительно обучалась на реальных данных. Поэтому фактор, связанный с {a}, рассматривается как измеримый источник риска, аспект, связанный с {b}, — как причина возможной ненадежности, а условие, связанное с {c}, — как основа повторяемой оценки.",
        f"Раздел «{topic}» также помогает подготовиться к вопросам защиты. Если спросят о практической значимости, ответ строится вокруг фактора, связанного с {a}; если спросят об ограничениях, нужно ссылаться на аспект, связанный с {b}; если спросят о воспроизводимости, ключевым становится условие, связанное с {c}.",
        f"Внутренняя логика темы «{topic}» не требует громких утверждений. Достаточно показать, что фактор, связанный с {a}, влияет на риск, аспект, связанный с {b}, меняет доверие к сцене, а условие, связанное с {c}, позволяет повторить эксперимент и проверить вывод «{d}».",
        f"Для текста ВКР раздел «{topic}» выполняет роль связки между обзором и реализацией. Он объясняет, почему фактор, связанный с {a}, не является второстепенной деталью, зачем учитывается аспект, связанный с {b}, и каким образом условие, связанное с {c}, попадает в проверяемую часть работы.",
        f"Практический смысл темы «{topic}» проявляется в том, что по ней можно задать проверочный вопрос и найти ответ в артефактах. В данных отражается фактор, связанный с {a}, в модели учитывается аспект, связанный с {b}, а в процедуре оценки закрепляется условие, связанное с {c}.",
    ]
    conclusions = [
        f"Для дальнейших разделов важно, что {d}. Этот результат используется при построении признаков reliability, uncertainty и risk score, а также при выборе метрик. Приоритет отдан recall и FNR, потому что пропуск критической сцены опаснее ложной тревоги, хотя частые false positive также снижают доверие к системе.",
        f"Таким образом, {d}. Этот вывод задает логику следующего шага: перейти от описания проблемы к признакам, модели и проверке на test split. В работе сохраняется баланс между полнотой объяснения и честным указанием ограничений данных.",
        f"Следовательно, {d}. В дальнейшем это положение проверяется не декларативно, а через подготовку таблицы сценариев, обучение baseline и proposed моделей, сравнение метрик и анализ ошибок.",
        f"Итог раздела можно сформулировать так: {d}. На этой основе далее строится экспериментальная часть, где качество оценивается не только общей accuracy, но и способностью модели не пропускать критические сцены.",
        f"Практический вывод раздела состоит в том, что {d}. Дальше это проверяется через признаки, сохраненные метрики и разбор конкретных FP/FN случаев.",
        f"Для проектной части это означает, что {d}. Поэтому в эксперименте важны не только итоговые числа, но и возможность объяснить, почему модель ошиблась в отдельных сценах.",
        f"Раздел подводит к выводу: {d}. Именно это положение затем связывает подготовку данных, обучение модели и интерпретацию результатов.",
        f"Из рассмотренного материала следует, что {d}. Такая формулировка сохраняет связь с целью ВКР и не расширяет выводы за пределы проверенного эксперимента.",
        f"Главный результат раздела: {d}. В следующих частях этот результат переводится в расчетные признаки, пороги и метрики качества.",
        f"Для дальнейшей методики зафиксировано, что {d}. Это позволяет оценивать прототип не только по общей точности, но и по поведению на редких критических сценах.",
    ]
    return [
        openings[variant],
        *factor_blocks[variant],
        bridge_blocks[(variant + 3) % len(bridge_blocks)],
        review_blocks[(variant + 1) % len(review_blocks)],
        project_blocks[(variant + 5) % len(project_blocks)],
        conclusions[(variant + 7) % len(conclusions)],
    ]
