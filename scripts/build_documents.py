...
def add_appendices(doc: Document, m: dict) -> None:
    ...
    appendices = [
        ("ПРИЛОЖЕНИЕ А", "Задание на ВКР", ["Задание подготовлено отдельным файлом Marianovskiy_zadanie_na_VKR_final_submission.docx и включено в работу как справочное приложение. Поля подписей и даты оставлены пустыми для заполнения по официальной процедуре."]),
        ...
        ("ПРИЛОЖЕНИЕ И", "Предметный указатель компетенций", ["Предметный указатель компетенций является последним приложением к работе. Подробная таблица также подготовлена отдельным файлом Marianovskiy_competency_index_final_submission.docx."]),
    ]
...
def add_chapter_2_clean(doc: Document) -> None:
    ...
    sections = sections[:-1] + [
        ...
        (
            ...
            [
                ...
                "Такая интерпретация нужна для защиты результатов. Работа не обещает готовую систему предотвращения аварий, но показывает, как выбранные признаки меняют recall, F1 и количество пропущенных сцен на фиксированном test split.",
            ],
        ),
        ("2.15 Выводы по главе 2", conclusion[1]),
    ]
    extend_section(sections, "2.12", [
        "В практической реализации правило critical_scene хранится не как скрытая экспертная договоренность, а как часть кода подготовки данных. Это важно для проверки: если преподаватель или другой исследователь заново выполнит подготовку KITTI-таблицы, он увидит не только итоговую метку, но и условия, из которых она получена.",
    ...
    extend_section(sections, "2.14", [
        ...
        "Матрица ошибок нужна комиссии и разработчику по разным причинам. Комиссии она показывает, что числа в тексте не являются абстрактными метриками; разработчику она сразу показывает, где цена ошибки выше.",
...
def add_chapter_3_clean(doc: Document, m: dict, summary: dict) -> None:
    ...
    sections = sections + [
        ...
        (
            ...
            [
                ...
                "Такой порядок делает работу проверяемой для преподавателя и комиссии. Можно открыть README, выполнить команды полного цикла и сопоставить полученные файлы с итоговыми материалами подачи.",
            ],
        ),
        (
            ...
            [
                ...
                "RX 7700 XT полезна как локальный ресурс для будущих моделей, но текущие метрики не зависят от GPU. Такое разделение важно для честности: наличие видеокарты не должно создавать впечатление, что в ВКР уже обучена тяжелая нейросетевая модель.",
    ...
    extend_section(sections, "3.14", [
        ...
        "Скрипт сборки документов не должен создавать публичные чек-листы и отчеты внутренней проверки. Такие материалы полезны локально, но в репозитории для защиты они отвлекают от воспроизводимого проекта.",
        ...
        "Репликация также ограничена лицензией и доступностью KITTI. В репозиторий не добавляются тяжелые исходные архивы, но структура путей и ожидаемые выходные файлы описаны так, чтобы проверяющий мог повторить подготовку на своей копии данных.",
    ])
    extend_section(sections, "3.15", [
        ...
        "GPU RX 7700 XT может помочь при обучении или инференсе более тяжелых моделей, но выбор backend на Windows нужно проверять отдельно. Нельзя переносить текущие CPU-метрики на будущую нейросетевую архитектуру.",
...
def build_report() -> Path:
    ...
    path = SUBMISSION / "Marianovskiy_VKR_ADAS_final_submission.docx"
    doc.save(path)
    return path
...
def build_assignment() -> Path:
    ...
    SUBMISSION.mkdir(exist_ok=True)
    path = SUBMISSION / "Marianovskiy_zadanie_na_VKR_final_submission.docx"
    doc.save(path)
    return path
...
def build_competency_index() -> Path:
    ...
    p(doc, "Руководитель ВКР: ______________________", first_line=False)
    SUBMISSION.mkdir(exist_ok=True)
    path = SUBMISSION / "Marianovskiy_competency_index_final_submission.docx"
    doc.save(path)
    return path
